...
int dso::DorisObsRinex::read_header() noexcept {
  ...
  /* check stream, go to top-of-file if needed */
  ...
  /* first line; RINEX VERSION / TYPE (get version) */
  ...
  /* second line; PGM / RUN BY / DATE (only validate, store nothing) */
  m_stream.getline(line, MAX_HEADER_CHARS);
  ...
  while (m_stream.getline(line, MAX_HEADER_CHARS) && !error) {
    if (!std::strncmp(line + 60, "SATELLITE NAME", 14)) {
      ...
      if (!tmp_sz || tmp_sz >= 59) {
        ...
        std::memcpy(satellite_name(), line, tmp_sz);
        satellite_name()[tmp_sz] = '\0';
      }
      ...
      if (!tmp_sz || tmp_sz > 19) {
        ...
        std::memcpy(cospar_number(), line, tmp_sz);
        cospar_number()[tmp_sz] = '\0';
      }
      ...
      /* REC # / TYPE / VERS; 
       * get m_rec_chain, m_rec_type and m_rec_version  (err. code 70)
       */
      tmp_sz = count_length_reverse(line, 19);
      if (tmp_sz < 1 || tmp_sz >= 19)
        error = 71;
      std::memcpy(rec_chain(), line, tmp_sz);
      rec_chain()[tmp_sz] = '\0';
      tmp_sz = count_length_reverse(line + 20, 19);
      if (tmp_sz < 1 || tmp_sz >= 19)
        error = 72;
      std::memcpy(rec_type(), line + 20, tmp_sz);
      rec_type()[tmp_sz] = '\0';
      tmp_sz = count_length_reverse(line + 40, 19);
      if (tmp_sz < 1 || tmp_sz >= 19)
        error = 73;
      std::memcpy(rec_version(), line + 40, tmp_sz);
      rec_version()[tmp_sz] = '\0';

    } else if (!std::strncmp(line + 60, "ANT # / TYPE", 12)) {
      /* ANT # / TYPE; 
       * get and validate m_antenna_number and m_antenna_type (err. code 80)
       */
      tmp_sz = count_length_reverse(line, 19);
      if (tmp_sz < 1 || tmp_sz >= 19)
        error = 81;
      std::memcpy(antenna_number(), line, tmp_sz);
      antenna_number()[tmp_sz] = '\0';
      tmp_sz = count_length_reverse(line + 20, 19);
      if (tmp_sz < 1 || tmp_sz >= 19)
        error = 82;
      std::memcpy(antenna_type(), line + 20, tmp_sz);
      antenna_type()[tmp_sz] = '\0';
      if (std::strcmp(antenna_number(), "DORIS") ||
          std::strcmp(antenna_type  (), "STAREC"))
        error = 84;
      ...
      if ( (cres.ec != std::errc{}) || (obs_types_num >= 13)) {
        error = 112;
      }
      ...
      for (int code = 0; code < obs_types_num; code++) {
        ...
        try {
          /* may throw! */
          dso::DorisObservationType type(dso::char_to_dobstype(*s)); 
          int freq = 0;
          if (dso::dobstype_has_frequency(type)) {
      ...
      /* TIME OF FIRST OBS 
       * get m_time_of_first_obs and validate time system (err. code 120)
       */
      ...
      if ((line[8] == line[9]) && (line[8] == ' ')) { // blank means all ....
        ...
        /* scale factors can be defined for any number of 
         * DorisObservationCodes, in the range [0, m_obs_codes.size()]
         */
      ...
      for (int code = 0; code < num_obs; code++) {
        ...
        try {
          ...
          dso::DorisObservationCode tmp(type, freq);
          ...
          auto it = std::find_if(
              ...
              [tmp](const dso::DorisObservationCode &o) { return o == tmp; });
      ...
      m_stations.emplace_back(dso::doris_rnx::Beacon{});
      ...
      /* TIME REF STATION; 
       * collect time reference station. make sure reference station is 
      ...
      if (auto it = std::find_if(m_stations.cbegin(), m_stations.cend(),
                                 [&refsta](const dso::doris_rnx::Beacon &s) {
                                   return !std::strcmp(
                                       s.code(), refsta.code());
                                 });
          it == m_stations.cend())
